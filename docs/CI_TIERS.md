...
### Rows still not present
  ...
  **Settled** — `src/world/hash.zig`. It hashes a **canonical logical projection**, not chunk bytes. Chunk bytes were the obvious candidate and are illegal: §0 P1 permits physical layout to differ per target and `chunk.Budget` makes it actually differ, so two *conformant* peers hold different chunk bytes for the same world by design. Hashing them would report a desync on every cross-platform session — the exact inverse of the bug the flag exists to find.
  ...
  The projection sorts entities by stable identity rather than storage order (two peers can legitimately differ on storage order after a scoped rollback re-simulates one island and not another), sorts components by stable registry ID, writes every scalar explicitly little-endian, and covers only the live prefix — including the dead tail would make the digest depend on per-target chunk capacity and reintroduce the problem by the back door. Floats in hashed state are a compile error.

  The digest is pinned and verified byte-identical on x86_64, aarch64, s390x, arm and mips by the row above. That verification is the only thing that makes the endianness claim worth anything, since all six shipping targets are little-endian.

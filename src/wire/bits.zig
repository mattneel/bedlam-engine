...
pub const Writer = struct {
    ...
    pub fn writeBits(self: *Writer, value: u64, bits: u7) Error!void {
        ...
        if (bits == 0) return;
        if (self.bit_pos + bits > self.buf.len * 8) return error.NoSpace;
    ...
    pub fn bitsWritten(self: Writer) usize {
        return self.bit_pos;
    }
...
pub const Reader = struct {
    ...
    pub fn init(buf: []const u8) Reader {
        return .{ .buf = buf, .bit_limit = buf.len * 8 };
    }
    ...
    pub fn bitsRead(self: Reader) usize {
        return self.bit_pos;
    }
};
...
test "zero-width reads and writes are identities" {
    ...
    try std.testing.expectEqual(@as(usize, 0), w.bitsWritten());

    var r = Reader.init(&buf);
    try std.testing.expectEqual(@as(u64, 0), try r.readBits(0));
    try std.testing.expectEqual(@as(usize, 0), r.bitsRead());
}
